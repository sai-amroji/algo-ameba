...
// ─── Selection Sort ──────────────────────────────────────────────────────────
export const buildSelectionSortFrames = (
  initialBars: SortBar[]
): SortFrame[] => {
  const bars = cloneBars(initialBars);
  const frames: SortFrame[] = [];
...
// ─── Insertion Sort ──────────────────────────────────────────────────────────
export const buildInsertionSortFrames = (
  initialBars: SortBar[]
): SortFrame[] => {
  const bars = cloneBars(initialBars);
  const frames: SortFrame[] = [];
  ...
  const partition = (low: number, high: number): number => {
    ...
    if (pivotFinalIdx !== high) {
      ...
      const liftOffsets: Record<string, { xFraction: number; yLevel: number }> =
        {};
      bars.forEach((b) => {
        liftOffsets[b.id] = { xFraction: 0, yLevel: 0 };
      });
      ...
      const settleOffsets: Record<
        string,
        { xFraction: number; yLevel: number }
      > = {};
      bars.forEach((b) => {
        settleOffsets[b.id] = { xFraction: 0, yLevel: 0 };
      });

      pushFrame(
        frames,
        bars,
        rangeStates(low, high, 'active', {
          [bars[pivotFinalIdx].id]: 'placed',
        }),
        0.6,
        settleOffsets
      );
  ...
  const sortRange = (low: number, high: number) => {
    if (low >= high) {
      ...
      if (low === high && !sortedIds.has(bars[low].id)) {
        ...
        const s: Record<string, SortBarState> = {};
        bars.forEach((b) => {
          s[b.id] = sortedIds.has(b.id) ? 'sorted' : 'default';
        });
        pushFrame(frames, bars, s, 0.5);
      }
      return;
...
// ─── Registry ────────────────────────────────────────────────────────────────
export const sortAlgorithmBuilders: Record<
  SortAlgorithmKey,
  (bars: SortBar[]) => SortFrame[]
> = {
