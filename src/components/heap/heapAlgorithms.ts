...
function pushToHeap(
  ...
  while (
    childIdx > 0 &&
    shouldSwap(newHeap[childIdx], newHeap[parentIdx], maxHeap)
  ) {
...
function popFromHeap(
  ...
  while (true) {
    ...
    if (
      leftChildIdx < newHeap.length &&
      shouldSwap(newHeap[leftChildIdx], newHeap[targetIdx], maxHeap)
    ) {
      targetIdx = leftChildIdx;
    }

    if (
      rightChildIdx < newHeap.length &&
      shouldSwap(newHeap[rightChildIdx], newHeap[targetIdx], maxHeap)
    ) {
    ...
    if (targetIdx !== parentIdx) {
      ...
      [newHeap[parentIdx], newHeap[targetIdx]] = [
        newHeap[targetIdx],
        newHeap[parentIdx],
      ];
      parentIdx = targetIdx;
