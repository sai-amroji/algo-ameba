...
const SearchPage = () => {
  ...
  const animateFoundCelebration = contextSafe((frame: SearchFrame) => {
    ...
    if (foundEl) {
      ...
      tl.to(foundEl, {
        ...
        duration: 0.55,
        ease:     "back.out(2.8)",
      }, 0.5); // starts while others are still sweeping back

      // 3. Pulse glow ring — expand a box-shadow then fade
      tl.to(foundEl, {
        boxShadow: "0 0 0 6px rgba(74,222,128,0.5), 0 0 28px rgba(74,222,128,0.35)",
        duration:  0.35,
        ease:      "power2.out",
      }, 0.8);
      tl.to(foundEl, {
        boxShadow: "0 0 0 3px rgba(74,222,128,0.2), 0 0 12px rgba(74,222,128,0.15)",
        duration:  0.6,
        ease:      "power1.inOut",
      }, 1.15);
    }
  ...
  const animateBinaryFrame = contextSafe((frame: SearchFrame) => {
    ...
    if (frame.isFinalFound) { animateFoundCelebration(frame); return; }
    if (frame.isNotFound)   { animateNotFound(frame);          return; }
    ...
    tl.to(activeEls, { opacity: 1, scale: 1, filter: "grayscale(0%)", duration: 0.25 }, 0);
    ...
    if (focusId) {
      ...
      if (focusEl) {
        ...
        tl.to(focusEl, { y: -20, scale: 1.14, duration: 0.4, ease: "back.out(2)" }, 0.28);
      }
    }
        ...
        gsap.fromTo(
          ...
          { drawSVG: "0%", opacity: 1 },
          {
            ...
            onComplete: () => {
              ...
              gsap.to(path, { opacity: 0, duration: 0.45, delay: 1.0, ease: "power1.in" });
            },
          }
        );
      });
